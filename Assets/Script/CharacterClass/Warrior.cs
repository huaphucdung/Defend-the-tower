...
public class Warrior : CharacterClass
{
    ...
    public override void DoAttack(PlayerController player, Animator anim, int combo) {
        ...
        switch (combo) {
            case 1:
                particle.SpawnObject(trans.position + new Vector3(0f, 0.5f, 0f), Quaternion.LookRotation(trans.forward), 0);
                dame -=12;
                break;
            case 2:
                particle.SpawnObject(trans.position + new Vector3(0f, 0.5f, 0f),  Quaternion.AngleAxis(220f, trans.forward) * Quaternion.LookRotation(trans.forward), 0);
                break;
            default:
                particle.SpawnObject(trans.position + new Vector3(0f, 0.5f, 0f) + trans.right * 0.3f, Quaternion.AngleAxis(250f, trans.forward) * Quaternion.LookRotation(trans.forward), 0);
                dame -=12;
                break;
        }
